...
<?PHP
	...
	$query1 = "SELECT password FROM LOGIN1 WHERE id='$id'";
	...
	if($old_password == $oldpassword)
	{
		...
		{
		$query = "UPDATE LOGIN1 SET password ='$newpassword' WHERE
		password ='$oldpassword' and id='$id'";
		$parseRequest = oci_parse($conn, $query);
		oci_execute($parseRequest);
		
		die( '<script>window.location = "MemberPage1.php";</script>');
		}
...
<form action='PasswordChange1.php' method='post'>
			Recent password: 
